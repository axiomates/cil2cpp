...
CIL2CPP 是 C# → C++ AOT 编译器（类似 Unity IL2CPP）。当前支持完整 C# 语法（100% IL 操作码覆盖），BCL 从 IL 编译（Unity IL2CPP 架构），~484 个 ICall 条目。1,291 C# + 600 C++ + 87 集成测试全部通过。首个 NuGet 包（Newtonsoft.Json 13.0.3）已完整验证。
...
| ICallRegistry 条目 | **~484 个**（涵盖 30+ 类别） |
| C# 编译器测试 | **1,291 个**（xUnit） |
| C++ 运行时测试 | **600 个**（Google Test，18 个测试文件） |
| 端到端集成测试 | **87 个**（14 个测试项目） |
| 运行时头文件 | **32 个** |
...
| Out/In 特性 | ❌ | 未区分参数方向（C.7.2） |
| 数组编组 / Ref String | ⚠️ | SizeParamIndex 已解析，codegen 待完善（C.7.3） |
...
完整编译管道：C# `.csproj` → codegen → CMake configure → C++ build → run → 验证输出。覆盖 14 个测试项目：HelloWorld、ArrayTest、FeatureTest、ArglistTest、MultiAssemblyTest、SystemIOTest、FileStreamTest、SocketTest、HttpGetTest、HttpTest、HttpsGetTest、DirTest、JsonSGTest、NuGetSimpleTest（Newtonsoft.Json 13.0.3）。同时包含 Debug 配置和类库项目测试。
