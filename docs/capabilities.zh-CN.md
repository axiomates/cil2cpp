...
CIL2CPP 是 C# → C++ AOT 编译器（类似 Unity IL2CPP）。当前支持完整 C# 语法（100% IL 操作码覆盖），BCL 从 IL 编译（Unity IL2CPP 架构），~484 个 ICall 条目。1,291 C# + 576 C++ + 87 集成测试全部通过。首个 NuGet 包（Newtonsoft.Json 13.0.3）已完整验证。
