...
CIL2CPP 是 C# → C++ AOT 编译器（类似 Unity IL2CPP）。当前支持完整 C# 语法（100% IL 操作码覆盖），BCL 从 IL 编译（Unity IL2CPP 架构），~490 个 ICall 条目。1,291 C# + 576 C++ + 171 集成测试全部通过。13 个 NuGet 包已验证（Newtonsoft.Json、DI+Logging+Console、Humanizer、Polly、Serilog+Console、Configuration+Binder、Hashids.net、GuardClauses、Slugify.Core）。Regex、DateTime、Decimal BCL 功能已验证。
...
| ICallRegistry 条目 | **~490 个**（涵盖 30+ 类别） |
| C# 编译器测试 | **1,291 个**（xUnit） |
| C++ 运行时测试 | **576 个**（Google Test，17 个测试文件） |
| 端到端集成测试 | **171 个**（28 个测试项目） |
| 运行时头文件 | **32 个** |
