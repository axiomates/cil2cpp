...
CIL2CPP 是 C# → C++ AOT 编译器（类似 Unity IL2CPP）。当前支持完整 C# 语法（100% IL 操作码覆盖），BCL 从 IL 编译（Unity IL2CPP 架构），~484 个 ICall 条目。1,273+ C# + 591 C++ + 35 集成测试全部通过。
...
完整编译管道：C# `.csproj` → codegen → CMake configure → C++ build → run → 验证输出。覆盖 HelloWorld、类库项目、Debug 配置、多程序集、变长参数、FeatureTest、SystemIO、FileStream、Socket 和 HttpClient 测试项目。
