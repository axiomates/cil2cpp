...
CIL2CPP 是 C# → C++ AOT 编译器（类似 Unity IL2CPP）。当前支持完整 C# 语法（100% IL 操作码覆盖），BCL 从 IL 编译（Unity IL2CPP 架构），~484 个 ICall 条目。1,291 C# + 600 C++ + 69 集成测试全部通过。
...
完整编译管道：C# `.csproj` → codegen → CMake configure → C++ build → run → 验证输出。覆盖 13 个测试项目：HelloWorld、ArrayTest、FeatureTest、SystemIO、FileStream、Socket、HttpGet、HttpsGet、HttpClient 构造、多程序集（含 MathLib 子目录）、Debug 配置、类库项目、NuGet (JsonSG)、变长参数。
