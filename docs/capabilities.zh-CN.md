...
CIL2CPP 是 C# → C++ AOT 编译器（类似 Unity IL2CPP）。当前支持完整 C# 语法（100% IL 操作码覆盖），BCL 从 IL 编译（Unity IL2CPP 架构），~484 个 ICall 条目。1,240 C# + 599 C++ + 47 集成测试全部通过。
...
| C++ 运行时测试 | **599 个**（Google Test，18 个测试文件） |
| 端到端集成测试 | **47 个**（11 个阶段） |
| 运行时头文件 | **32 个** |
