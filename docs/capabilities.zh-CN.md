...
CIL2CPP 是 C# → C++ AOT 编译器（类似 Unity IL2CPP）。当前支持完整 C# 语法（100% IL 操作码覆盖），BCL 从 IL 编译（Unity IL2CPP 架构），~490 个 ICall 条目。1,291 C# + 576 C++ + 93 集成测试全部通过。已验证 NuGet 包：Newtonsoft.Json 13.0.3 + DI 生态（DITest 含 3 个 NuGet 包）。
...
完整编译管道：C# `.csproj` → codegen → CMake configure → C++ build → run → 验证输出。覆盖 15 个测试项目：HelloWorld、ArrayTest、FeatureTest、ArglistTest、MultiAssemblyTest、SystemIOTest、FileStreamTest、SocketTest、HttpGetTest、HttpTest、HttpsGetTest、DirTest、JsonSGTest、NuGetSimpleTest（Newtonsoft.Json 13.0.3）、DITest（DI+Logging+Console）。同时包含 Debug 配置和类库项目测试。
