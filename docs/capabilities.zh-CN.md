...
CIL2CPP 是 C# → C++ AOT 编译器（类似 Unity IL2CPP）。当前支持完整 C# 语法（100% IL 操作码覆盖），BCL 从 IL 编译（Unity IL2CPP 架构），~395 个 ICall 条目。1,291 C# + 595 C++ + 204 集成测试全部通过。15 个 NuGet 包已验证（Newtonsoft.Json、DI+Logging+Console、Humanizer、Polly、Serilog+Console、Configuration+Binder、Hashids.net、GuardClauses、Slugify.Core、Stateless、FluentValidation）。Regex、DateTime、Decimal BCL 功能已验证。3 个真实项目验证（MiniCsvTool、TodoManager、HealthChecker）。
...
| ICallRegistry 条目 | **~395 个**（涵盖 30+ 类别） |
| C# 编译器测试 | **1,291 个**（xUnit） |
| C++ 运行时测试 | **595 个**（Google Test，18 个测试文件） |
...
| System.IO.Compression | ✅ | GZipStream/DeflateStream 通过 zlib（FetchContent），CompressionNative_* 互操作 |
| System.Text.RegularExpressions | ✅ | 解释器模式（非 Compiled）。IsMatch/Match/Replace/Split、命名组、RegexOptions、超时 |
| System.DateTime / TimeSpan | ✅ | DateTime.Now/UtcNow/Parse/ToString、TimeSpan 算术、DateTimeOffset、格式化 |
| System.Decimal | ✅ | 算术、Parse/TryParse、ToString、Math.Round/Floor/Ceiling |
| System.Net（Socket/DNS/HTTP） | ✅ | Socket TCP 环回 ✅、DNS 解析 ✅、HttpClient HTTP GET ✅、HTTPS GET ✅（Windows，Winsock/SChannel P/Invoke）。TLS 通过 OS 自带 SChannel。 |
...
FileStream / StreamReader / StreamWriter 从 BCL IL 编译并在 Windows 端到端运行（通过 kernel32 P/Invoke）。Linux 需要 System.Native 集成（Phase B.5）。
...
| MarshalAs 特性 | ✅ | Cecil MarshalInfo 解析，21 种类型映射（LPStr/LPWStr/Bool/整数等） |
| Out/In 特性 | ✅ | PInvokeDirection 完整解析，bool* copy-back，blittable 指针直接写入（C.7.2） |
| 数组编组 / Ref String | ✅ | LPArray 方向感知 + SizeParamIndex 调试断言，ByValTStr/ByValArray 内联数组（C.7.3） |
...
| System.Net（HTTPS 边界情况） | HttpClient HTTP/HTTPS GET 已通过；复杂场景（重定向、分块编码、认证）尚未验证 |
| Regex Compiled 模式 | Compiled 模式使用 Reflection.Emit（AOT 不兼容）；解释器模式可用 |
...
| HttpClient HTTP GET | 完整 | 未实现 | 未实现 | SocketsHttpHandler → Socket → ws2_32 P/Invoke |
| HttpClient HTTPS GET | 完整 | 未实现 | 未实现 | SslStream → SChannel (secur32/sspicli) P/Invoke |
...
| 反射 (TypeCode) | 完整 | 完整 | 完整 | 17 种基元类型正确映射到 TypeCode 枚举值（Phase H.1 ✅） |
| 反射 (IsPublic/可见性) | 完整 | 完整 | 完整 | TypeFlags::Public/NestedPublic 来自 Cecil 元数据（审计 ✅） |
| 反射 (StackFrame) | 占位 | 占位 | 占位 | 方法信息返回 nullptr |
| 反射 (Assembly 元数据) | 占位 | 占位 | 占位 | 无 Assembly 对象。需 Phase D |
| 线程优先级 | 占位 | 占位 | 占位 | No-op，返回 Normal |
| 线程 ManagedId | 完整 | 完整 | 完整 | 每线程唯一原子 ID（主线程=1，工作线程从 2 开始）。P0 修复：消除重复 ID 风险 |
...
> 23 个反射 ICall 中有 12 个返回简化/占位值。完整反射保真度需要 Phase D（NativeAOT 元数据）。

| ICall | 期望行为 (.NET) | 当前行为 (CIL2CPP) | 修复阶段 |
|-------|---------------|-------------------|---------|
| Type.GetTypeCode | 按类型返回枚举 (Int32→9, String→18 等) | 正确 ✅（17 种基元类型已映射） | ~~H.1~~ 已完成 |
| Type.IsPublic | 实际可见性 | 正确 ✅（TypeFlags::Public） | ~~H.1~~ 已完成 |
| Type.IsAbstract | 实际标志位 | 正确 ✅ | — |
| Type.IsValueType | 实际标志位 | 正确 ✅ | — |
| Type.IsArray | 实际标志位 | 正确 ✅ | — |
| Type.IsNestedPublic | 嵌套可见性 | 正确 ✅（TypeFlags::NestedPublic） | ~~审计~~ 已完成 |
