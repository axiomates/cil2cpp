...
| 14 个 NuGet 包验证 | G.2 | 证明真实包可编译和运行（14/14 冒烟测试通过，但覆盖极浅 — 见成熟度评估） |
...
**测试**：1,291 C# + 576 C++ + 177 集成（29 个测试项目）— 全部通过。
...
- **没有任何真实的大型复杂 .NET NativeAOT 项目被编译通过** — 全部 29 个测试项目都是玩具级别的示例
...
| NuGet 包 | ~70% | 极低 | 14 个包的最小示例通过；各库仅调用 2-5 个 API |
...
- NuGet PackageReference — 14 个包冒烟测试通过，但覆盖极浅
...
| # | 任务 | 预估 | 状态 | 说明 |
|---|------|------|------|------|
| D.0 | NuGet 包集成测试 | 中 | ✅ 已完成 | 14 个 NuGet 包冒烟测试通过（Newtonsoft.Json、DI+Logging+Console、Humanizer、Polly、Serilog+Console、Configuration+Binder、Hashids.net、GuardClauses、Slugify.Core、Stateless）。每个包使用 30-88 行最小示例验证，覆盖极浅。177/177 集成测试。 |
...
| G.2 | 14 NuGet 包冒烟测试 | 高 | ✅ 完成 | 14 个包通过最小示例测试（见成熟度评估中的详细列表）。注意：每个包只验证了极小 API 子集（<5%） |
...
| **M6: 生态深度** | 50+ NuGet 包无需手动修复即可编译 + 3 个真实应用验证 | G | 进行中（14 NuGet、Regex ✅、DateTime ✅、Decimal ✅） |
