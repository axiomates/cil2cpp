...
| 15 个 NuGet 包验证 | G.2 | 证明真实包可编译和运行（15/15 冒烟测试通过，但覆盖极浅 — 见成熟度评估） |
...
**测试**：1,291 C# + 595 C++ + 204 集成（34 个测试项目）— 全部通过。
...
- **没有任何真实的大型复杂 .NET NativeAOT 项目被编译通过** — 34 个测试项目中有 3 个真实项目验证（MiniCsvTool ~400 行、TodoManager ~550 行、HealthChecker ~500 行）
- **有限的编译器优化** — 已实现 __Canon 共享、并行 Pass 6、并行头文件生成、PCH、增量 VTable、mangled-name O(1) 索引、延迟方法特化体编译、增量被调用函数追踪（NuGetSimpleTest IRBuilder 62s→38s）；但没有内联、去虚拟化、逃逸分析
...
| NuGet 包 | ~70% | 极低 | 15 个包的最小示例通过；各库仅调用 2-5 个 API |
...
| 11 | Hashids.net | 1.7.0 | 62 行 | 整数编码/解码、十六进制编码、LINQ Intersect |
| 12 | Ardalis.GuardClauses | 4.6.0 | 70 行 | Guard.Against.Null/NullOrEmpty/OutOfRange/Zero/NegativeOrZero |
| 13 | Slugify.Core | 4.0.1 | 77 行 | GenerateSlug、特殊字符、自定义配置 |
| 14 | Stateless | 5.16.0 | ~60 行 | 状态机、泛型接口分派、6 个测试 |
| 15 | FluentValidation | 11.11.0 | ~80 行 | 表达式树编译（LightCompiler 解释器）、6 个测试 |
...
- NuGet PackageReference — 15 个包冒烟测试通过（M6 第二阶段加深 6 个测试套件）
- Codegen 性能 — NuGetSimpleTest IRBuilder 62s→38s（在 89s 基础上通过 mangled-name O(1) 索引、延迟方法特化体编译、增量被调用函数追踪再减 39%）；总 codegen ~52s
- **编译器优化** — 部分：__Canon 共享、并行 Pass 6、并行头文件生成、PCH、增量 VTable、mangled-name O(1) 索引、延迟方法特化体编译、增量被调用函数追踪。无内联/去虚拟化/逃逸分析
...
| # | 任务 | 预估 | 状态 | 说明 |
|---|------|------|------|------|
| D.0 | NuGet 包集成测试 | 中 | ✅ 已完成 | 15 个 NuGet 包冒烟测试通过（Newtonsoft.Json、DI+Logging+Console、Humanizer、Polly、Serilog+Console、Configuration+Binder、Hashids.net、GuardClauses、Slugify.Core、Stateless、FluentValidation）。M6 第二阶段加深 6 个套件。204/204 集成测试（34 个项目）。 |
...
| G.2 | 15 NuGet 包冒烟测试 | 高 | ✅ 完成 | 15 个包通过最小示例测试（见成熟度评估中的详细列表）。注意：每个包只验证了极小 API 子集（<5%） |
...
| **M6: 生态深度** | 50+ NuGet 包无需手动修复即可编译 + 3 个真实应用验证 | G | 进行中（15 NuGet、Regex ✅、DateTime ✅、Decimal ✅、FluentValidation ✅、3 个真实项目 ✅） |
...
| ICall 数 | C++ 内部调用 | **~395** | ~400 | 趋稳（功能来自 BCL IL，非 ICall） |
