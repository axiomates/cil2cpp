...
public partial class CppCodeGenerator
{
    private GeneratedFile GenerateHeader()
    {
        ...
        // Parallel: each method writes to its own ReferencedTypeInfoNames/ReferencedPointerTypeNames sets.
        var methodsWithBodies = userTypes.SelectMany(t => t.Methods).Where(m => m.BasicBlocks.Count > 0).ToList();
        Parallel.ForEach(methodsWithBodies, m => m.ComputeTypeReferences());
        ...
        var fwdDeclMs = headerSw.ElapsedMilliseconds;
        ...
        var typeInfoDeclMs = headerSw.ElapsedMilliseconds - fwdDeclMs;
        ...
        // Pre-filter to types that will be emitted (deterministic order from topological sort)
        var typesToEmit = new List<IRType>();
        ...
        // Parallel generate struct definitions — each type's definition is independent.
        // GenerateStructDefinition only reads definedTypeNames (via SanitizeFieldType.Contains).
        var structDefs = new string[typesToEmit.Count];
        Parallel.For(0, typesToEmit.Count, i =>
        {
            var localSb = new StringBuilder();
            GenerateStructDefinition(localSb, typesToEmit[i], definedTypeNames);
            structDefs[i] = localSb.ToString();
        });
        // Concatenate in topological order (deterministic output)
        foreach (var def in structDefs)
            sb.Append(def);

        var typeDefMs = headerSw.ElapsedMilliseconds - fwdDeclMs - typeInfoDeclMs;
        ...
        var methodDeclMs = headerSw.ElapsedMilliseconds - fwdDeclMs - typeInfoDeclMs - typeDefMs;
        ...
        var stubsMs = headerSw.ElapsedMilliseconds - fwdDeclMs - typeInfoDeclMs - typeDefMs - methodDeclMs;
        Console.Error.WriteLine($"[perf] Header sections: fwdDecl={fwdDeclMs}ms typeInfoDecl={typeInfoDeclMs}ms " +
            $"typeDef={typeDefMs}ms methodDecl={methodDeclMs}ms stubs={stubsMs}ms total={headerSw.ElapsedMilliseconds}ms");
        ...
        List<IR.IRType> userTypes, HashSet<string> knownTypeNames)
    {
        ...
        // Parallel: each type collects its called functions independently, then merge.
        var calledFunctions = new HashSet<string>();
        {
            var nonDelegateTypes = userTypes.Where(t => !t.IsDelegate).ToList();
            var perTypeResults = new HashSet<string>[nonDelegateTypes.Count];
            Parallel.For(0, nonDelegateTypes.Count, i =>
            {
                var type = nonDelegateTypes[i];
                var localCalled = new HashSet<string>();
                    ...
                    foreach (var block in method.BasicBlocks)
                    {
                        foreach (var instr in block.Instructions)
                        {
                            if (instr is IR.IRCall call && !string.IsNullOrEmpty(call.FunctionName)
                                ...
                                localCalled.Add(ldftn.MethodCppName);
                        }
                    }
                }
                perTypeResults[i] = localCalled;
            });
            foreach (var localSet in perTypeResults)
                foreach (var name in localSet)
