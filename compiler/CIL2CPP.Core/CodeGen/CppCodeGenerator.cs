...
public partial class CppCodeGenerator
{
    ...
    /// For out-of-module functions, falls back to shared DeadCodePatterns table.
    /// </summary>
    ...
    /// <summary>
    /// Generate precompiled header (pch.h) containing all common includes.
    /// CMake's target_precompile_headers compiles this once per configuration,
    /// eliminating redundant header parsing across all translation units.
    /// </summary>
    private GeneratedFile GeneratePch()
    {
        var sb = new StringBuilder();
        sb.AppendLine("// Generated by CIL2CPP - DO NOT EDIT");
        sb.AppendLine("// Precompiled header: common includes for all source files");
        sb.AppendLine();
        sb.AppendLine("#pragma once");
        sb.AppendLine();
        sb.AppendLine("// Project header (includes <cil2cpp/cil2cpp.h> umbrella)");
        sb.AppendLine($"#include \"{_module.Name}.h\"");
        sb.AppendLine();
        sb.AppendLine("// Standard library headers used by generated code");
    ...
    private GeneratedFile GenerateCMakeLists(GeneratedOutput output)
    {
        ...
        // Precompiled header: compile pch.h once, reuse across all translation units
        sb.AppendLine($"target_precompile_headers({projectName} PRIVATE pch.h)");
        sb.AppendLine();
    ...
    /// Uses the shared DeadCodePatterns table (single source of truth with ClassifyMethodDeadCode).
    /// </summary>
    private static DeadCodeCategory ClassifyDeadCodeByName(string functionName)
    {
        foreach (var (ilPrefix, category, _) in DeadCodePatterns.TypePatterns)
        {
            var cppPrefix = DeadCodePatterns.ToCppPrefix(ilPrefix);
            if (functionName.StartsWith(cppPrefix))
                return category;
        }
        return DeadCodeCategory.None;
    }
...
public class GeneratedOutput
{
    ...
    /// <summary>
    /// Precompiled header file: common includes shared by all source files.
    /// CMake's target_precompile_headers compiles this once, eliminating
    /// redundant header parsing across all translation units.
    /// </summary>
    public GeneratedFile PchFile { get; set; } = new();
    ...
    public void WriteToDirectory(string outputDir)
    {
        ...
        var generatedFileNames = new HashSet<string>(StringComparer.OrdinalIgnoreCase)
            { HeaderFile.FileName, PchFile.FileName };
        ...
        File.WriteAllText(Path.Combine(outputDir, PchFile.FileName), PchFile.Content);
